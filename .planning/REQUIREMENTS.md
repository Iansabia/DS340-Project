...
Adversarial audit of every quantitative claim before April 27 submission. Kill-or-confirm posture: each claim either survives unchanged (with documented assumptions), is corrected (with paper updated in same wave), or is removed. Headline target: defend or correct the per-pair Sharpe ≈ 3.2 backtest number. No new model training; this audits Phase 17's canonical results.

- [x] **AUDIT-01**: `experiments/audit/audit_sharpe.py` recomputes per-trade Sharpe (≈ 0.04) and per-pair Sharpe (≈ 3.2) from raw test-set trade ledger; reports bootstrap 95% CI (10,000 resamples) on both; computes cross-sectional pair-correlation correction and reports the corrected Sharpe; writes `experiments/results/audit/sharpe_audit.json` with the full assumption stack (annualization factor N, n_pairs, n_eff, avg_pair_corr, CI bounds). PAPER_DRAFT.md abstract / §5 / Table 8 updated to include CI lower bound and corrected Sharpe.
- [x] **AUDIT-02**: `experiments/audit/audit_leakage.py` classifies every feature in `src/features/build_features.py` as Safe / Suspicious / Leaking with evidence (line number, rolling-window endpoint, label-alignment check). Walk-forward embargo verified: for each window, no pair_id is open in both train end and test start. Matching pipeline `src/matching/quality_filter.py` audited rule-by-rule for retroactive use of post-resolution information. Output: `experiments/results/audit/leakage_audit.json` with per-feature classification and per-window embargo report.
- [x] **AUDIT-03**: `experiments/audit/audit_costs.py` confirms Kalshi maker/taker fees are charged per trade in `simulate_profit` and `WalkForwardBacktester`; recomputes net Sharpe and net P&L if fees were missing or wrong; runs a slippage sensitivity (0bps / 5bps / 10bps haircut) and reports how much of the headline edge survives each. Polymarket gas + withdrawal cost assumption explicitly documented in PAPER_DRAFT.md §6.4.
- [x] **AUDIT-04**: `experiments/audit/audit_survivorship.py` audits pair universe construction by cross-referencing the full pair history in `data/processed/pairs/` against the filter applied at training time. Spot-checks 10 randomly-dropped pairs and confirms each drop reason is structural (insufficient overlap, low liquidity), not retroactive. Output: `experiments/results/audit/survivorship_audit.json` with the random sample + reason classification.
- [x] **AUDIT-05**: `experiments/results/audit/paper_numbers.csv` enumerates every numeric claim in PAPER_DRAFT.md (one row per claim: claim_text, paper_section, source_file, source_command, recomputed_value, match_status). `scripts/check_paper.sh` extended with at least 5 new regression checks that recompute headline numbers from canonical files and grep-match them in PAPER_DRAFT.md.
- [x] **AUDIT-06**: `AUDIT_REPORT.md` at project root presents one row per audit dimension (Tier 1–6 from CONTEXT.md), each marked PASS / CORRECTED / FAILED with linked evidence (script path, JSON output, recomputed number). If any row is CORRECTED, PAPER_DRAFT.md and `slides_deck.html` are updated in the same plan that produces the correction. If all rows are PASS, `AUDIT_REPORT.md` is referenced from PAPER_DRAFT.md §6.4 as supplementary evidence of methodological care.
- [x] **AUDIT-07**: Pair-stratified leakage-free recompute (added 2026-04-26 after AUDIT-02 found 142/144 pairs bridge canonical 80/20 split). `experiments/run_canonical_purged.py` retrains LR + XGBoost on a pair-stratified train/test split (seed=42, 80/20 by `pair_id`, no pair appears in both halves) and writes `experiments/results/canonical_purged/headline.json`. `experiments/audit/audit_sharpe_purged.py` recomputes per-trade Sharpe + per-pair Sharpe + bootstrap 95% CI + BLdP correlation correction on the purged results and writes `experiments/results/audit/sharpe_audit_purged.json`. A side-by-side comparison table in `AUDIT_REPORT.md` documents leaky-canonical vs purged numbers; the purged Sharpe becomes the new headline in PAPER_DRAFT.md abstract / §5 / Table 8 / §8 Conclusions and `slides_deck.html`.
