...
- [ ] **AUDIT-05**: `experiments/results/audit/paper_numbers.csv` enumerates every numeric claim in PAPER_DRAFT.md (one row per claim: claim_text, paper_section, source_file, source_command, recomputed_value, match_status). `scripts/check_paper.sh` extended with at least 5 new regression checks that recompute headline numbers from canonical files and grep-match them in PAPER_DRAFT.md.
