...
- [ ] **AUDIT-01**: `experiments/audit/audit_sharpe.py` recomputes per-trade Sharpe (≈ 0.04) and per-pair Sharpe (≈ 3.2) from raw test-set trade ledger; reports bootstrap 95% CI (10,000 resamples) on both; computes cross-sectional pair-correlation correction and reports the corrected Sharpe; writes `experiments/results/audit/sharpe_audit.json` with the full assumption stack (annualization factor N, n_pairs, n_eff, avg_pair_corr, CI bounds). PAPER_DRAFT.md abstract / §5 / Table 8 updated to include CI lower bound and corrected Sharpe.
