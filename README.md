...
The **research decisions** in this project — the central question, which model tiers to compare, which hyperparameters to sweep, the evaluation regimes (single-split, walk-forward, data-scaling, transaction-cost sensitivity, live deployment), the matching-pipeline rules, the reward function for PPO, the audit methodology, and the interpretation of every result — are by the human authors (Ian Sabia, Alvin Jang). The **implementation code** throughout this repository was developed with AI pair-programming assistance (Anthropic Claude via Claude Code), with the human authors specifying requirements, reviewing every change, and testing behavior. This applies to the model wrappers in `src/models/` as well as to the supporting infrastructure (data ingestion, market matching, feature engineering, evaluation, live trading system, audit pipeline, tests, scripts). Each package's `__init__.py` carries a short docstring identifying its authorship category. No code was copied from online sources; all third-party libraries (`scikit-learn`, `xgboost`, `pytorch`, `pytorch-forecasting`, `stable_baselines3`, `sentence-transformers`, etc.) are used per their published licenses.
